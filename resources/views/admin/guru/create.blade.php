<x-tahfidz-layout>
    ...
    <div class="max-w-2xl">
        <div class="mb-6">
            <a href="{{ route('admin.guru.index') }}" class="text-sm font-bold text-emerald-700 dark:text-emerald-500 flex items-center gap-2 hover:gap-3 transition-all">
        ...
        <form action="{{ route('admin.guru.store') }}" method="POST" class="space-y-6">
            ...
            <x-tahfidz.card title="Informasi Dasar Guru">
                <div class="space-y-4">
                    <x-tahfidz.form-input name="name" label="Nama Lengkap" placeholder="Masukkan nama lengkap ustadz/ustadzah" required />
                    <x-tahfidz.form-input type="email" name="email" label="Alamat Email (Digunakan untuk Login)" placeholder="guru@example.com" required />
                    <x-tahfidz.form-input type="number" name="phone" label="Nomor Handphone / WhatsApp" placeholder="Contoh: 08123456789" required />
                </div>
            </x-tahfidz.card>

            <x-tahfidz.card title="Akses & Keamanan">
                <p class="text-xs text-gray-500 mb-4">Jika Anda membiarkan form sandi ini kosong, sistem secara otomatis akan mengatur sandi mengunakan <span class="font-bold">Nomor Handphone</span> beliau.</p>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-tahfidz.form-input type="password" name="password" label="Kata Sandi (Opsional)" placeholder="••••••••" />
                    <x-tahfidz.form-input type="password" name="password_confirmation" label="Konfirmasi Kata Sandi" placeholder="••••••••" />
                </div>
            </x-tahfidz.card>
