<x-tahfidz-layout>
    ...
    <x-slot name="subtitle">
        Perbarui informasi profil akun guru.
    </x-slot>
    ...
    <div class="max-w-2xl">
        ...
        @if($errors->any())
            <div class="mb-6 p-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-2xl flex items-start gap-3 animate-fadeIn">
                ...
                <div>
                    <h3 class="text-sm font-bold text-red-800 dark:text-red-300">Pembaruan Gagal!</h3>
        ...
        <form action="{{ route('admin.guru.update', $guru) }}" method="POST" class="space-y-6">
            ...
            <x-tahfidz.card title="Reset Sandi (Opsional)">
                <p class="text-xs text-gray-500 mb-4">Abaikan form ini jika Anda tidak ingin mengubah kata sandi.</p>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-tahfidz.form-input type="password" name="password" label="Kata Sandi Baru" placeholder="••••••••" />
                    <x-tahfidz.form-input type="password" name="password_confirmation" label="Konfirmasi Sandi Baru" placeholder="••••••••" />
                </div>
            </x-tahfidz.card>

            <div class="flex justify-end pt-4">
                <button type="submit" class="px-8 py-4 bg-blue-700 text-white rounded-2xl font-bold text-lg hover:bg-blue-800 shadow-xl shadow-blue-200 dark:shadow-none transition-all flex items-center gap-3">
