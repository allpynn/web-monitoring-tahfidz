<x-tahfidz-layout>
    ...
    <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="relative w-full md:w-96">
            ...
            <input type="text" x-model="search" class="block w-full pl-12 pr-4 py-3 border border-gray-100 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-800 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm dark:text-white transition-all shadow-sm" placeholder="Cari user berdasarkan nama/email/HP...">
        </div>
        <a href="{{ route('admin.users.create') }}" class="w-full md:w-auto px-6 py-3 bg-emerald-700 text-white rounded-2xl text-sm font-bold hover:bg-emerald-800 transition-all shadow-lg shadow-emerald-200 dark:shadow-none flex items-center justify-center">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr x-show="!search || '{{ strtolower($user->name) }}'.includes(search.toLowerCase()) || '{{ strtolower($user->email) }}'.includes(search.toLowerCase()) || '{{ strtolower($user->role) }}'.includes(search.toLowerCase())" 
                        ...
                        <td class="px-8 py-5">
                            <span class="px-3 py-1 text-xs font-bold rounded-lg uppercase tracking-wider {{ $user->role === 'guru' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400' : 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-400' }}">
                                {{ str_replace('_', ' ', $user->role) }}
                            </span>
                        </td>
                        <td class="px-8 py-5 text-center text-xs">
                            <div class="flex justify-center items-center space-x-3">
                                <a href="{{ route('admin.users.edit', $user) }}" class="p-2.5 text-gray-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded-xl transition-all" title="Ubah">
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-8 py-20 text-center text-gray-400 dark:text-gray-500">Belum ada user terdaftar.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
