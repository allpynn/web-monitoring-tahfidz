    ...
    <div class="max-w-3xl">
        ...
        <form action="{{ route('admin.students.store') }}" method="POST" class="space-y-6">
            ...
            <x-tahfidz.card title="Data Orang Tua / Wali">
                <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">Masukkan data orang tua secara manual. Sistem akan otomatis membuat akun orang tua.</p>
                ...
                <div id="parent-container" class="space-y-4">
                    <div class="parent-entry p-4 bg-gray-50 dark:bg-gray-900/40 rounded-2xl border border-gray-100 dark:border-gray-700 space-y-4">
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Orang Tua #1</span>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Nama Lengkap</label>
                                <input type="text" name="parent_names[]" value="{{ old('parent_names.0') }}" required placeholder="Masukkan nama orang tua" class="block w-full px-4 py-3 border border-gray-100 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-800 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm dark:text-white transition-all shadow-sm placeholder-gray-400">
                                @error('parent_names.0') <p class="mt-1 text-xs text-red-600 font-bold italic">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Nomor Telepon</label>
                                <input type="text" name="parent_phones[]" value="{{ old('parent_phones.0') }}" required placeholder="Contoh: 08123456789" class="block w-full px-4 py-3 border border-gray-100 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-800 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm dark:text-white transition-all shadow-sm placeholder-gray-400">
                ...
                @error('parent_names') <p class="mt-2 text-xs text-red-600 font-bold italic">{{ $message }}</p> @enderror

                <button type="button" id="add-parent-btn" class="mt-4 w-full py-3 border-2 border-dashed border-emerald-300 dark:border-emerald-700 text-emerald-600 dark:text-emerald-400 rounded-2xl text-sm font-bold hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all flex items-center justify-center gap-2">
            ...
            <x-tahfidz.card title="Guru Pendamping">
                <div class="w-full">
                    <label for="guru_id" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Pilih Guru Pendamping</label>
                    <select name="guru_id" id="guru_id" required class="block w-full px-4 py-3 border border-gray-100 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-800 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm dark:text-white transition-all shadow-sm">
