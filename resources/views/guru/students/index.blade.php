<x-tahfidz-layout>
    ...
    <x-slot name="subtitle">
        Menampilkan santri yang diampu pada tahun ajaran <span
            class="font-bold text-emerald-600">{{ $academicYear }}</span>
    </x-slot>

    @if(session('success'))
        <div
            class="mb-6 p-4 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-2xl border border-emerald-100 dark:border-emerald-800 font-bold flex items-center gap-3 animate-fadeIn">
    ...
    <div
        class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-3xl overflow-hidden shadow-sm">
        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700 bg-gray-50/30 dark:bg-gray-900/10">
            <form action="{{ route('guru.students.index') }}" method="GET"
                class="flex flex-col lg:flex-row gap-4 items-end">
                {{-- SEARCH: Kiri, lebar --}}
                <div class="flex-1 w-full">
                    <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1 tracking-widest">Cari
                        Nama / NIS</label>
                    <div class="relative">
                ...
                <div class="w-full lg:w-48">
                    <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1 tracking-widest">Tahun
                        Ajaran</label>
                ...
                <div class="w-full lg:w-40">
                    <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1 tracking-widest">Jenis
                        Kelamin</label>
                ...
                <div class="w-full lg:w-40">
                    <label
                        class="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1 tracking-widest">Urutkan</label>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-gray-50/50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700">
                            <th
                                class="px-6 py-3 text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest leading-none">
                                Info Santri</th>
                            <th
                                class="px-6 py-3 text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest leading-none">
                                Target</th>
                            <th
                                class="px-6 py-3 text-[10px] font-black text-emerald-600 uppercase tracking-widest leading-none">
                                Terverifikasi</th>
                            <th
                                class="px-6 py-3 text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest leading-none text-right">
                                Aksi</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-900/30 transition-colors group">
                                <td class="px-6 py-2.5">
                                    <div class="flex items-center gap-4">
                                        <div
                                            class="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 flex items-center justify-center font-bold text-lg shadow-sm">
                                            {{ substr($student->name, 0, 1) }}
                                        </div>
                                        <div>
                                            <div class="text-sm font-bold text-gray-900 dark:text-white leading-none mb-1">
                                                {{ $student->name }}
                                            </div>
                                            <div class="text-[10px] text-gray-500 font-bold italic tracking-tighter">NIS:
                                ...
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-wrap gap-1.5 max-w-[200px]">
                                        ...
                                        @forelse($student->targets as $t)
                                            ...
                                            <span
                                                class="px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-tighter {{ $isAchieved ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-blue-50 text-blue-600 border border-blue-100' }}">
                                                JUZ {{ $t->target_juz }}
                                            </span>
                                        @empty
                                ...
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-wrap gap-1">
                                        @forelse($student->completed_juz as $cj)
                                            <span
                                                class="px-2 py-0.5 bg-emerald-600 text-white text-[9px] font-black rounded uppercase">Juz
                                                {{ $cj }}</span>
                                        @empty
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        <p class="text-gray-400 italic font-medium text-sm">Tidak ada santri yang diampu
                                            pada tahun ajaran <span class="font-bold">{{ $academicYear }}</span>.</p>
                                        <p class="text-xs text-gray-300">Coba pilih tahun ajaran lain atau "Semua Tahun".
                                        </p>
                                    </div>
                                </td>
                            </tr>
