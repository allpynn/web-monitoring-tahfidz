<x-tahfidz-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <a href="{{ route('guru.hafalan.create') }}" class="p-4 bg-emerald-600 dark:bg-emerald-700 text-white rounded-2xl flex items-center gap-4 hover:bg-emerald-700 transition shadow-lg">
        ...
        <a href="{{ route('guru.students.index') }}" class="p-4 bg-blue-600 dark:bg-blue-700 text-white rounded-2xl flex items-center gap-4 hover:bg-blue-700 transition shadow-lg">
            <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
            </div>
            <div>
                <span class="block font-bold text-lg">Data Santri</span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        ...
        <x-tahfidz.card title="Total Santri" :value="$stats['total_santri']" icon='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>' />
    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-4">
        ...
        <form action="{{ route('guru.dashboard') }}" method="GET" class="flex items-center gap-4 bg-white/80 dark:bg-gray-800/80 p-2 rounded-xl border border-gray-100 dark:border-gray-700">
            <select name="month" class="rounded-lg border-gray-200 dark:border-gray-700 bg-transparent text-xs font-bold" onchange="this.form.submit()">
                @foreach(range(1, 12) as $m)
                    <option value="{{ $m }}" {{ $month == $m ? 'selected' : '' }}>{{ \Carbon\Carbon::create($year, $m, 1)->translatedFormat('F') }}</option>
                @endforeach
            </select>
            <select name="year" class="rounded-lg border-gray-200 dark:border-gray-700 bg-transparent text-xs font-bold" onchange="this.form.submit()">
        ...
        <x-tahfidz.card title="⚠️ Perhatian Khusus (Alert)" class="border-red-200 dark:border-red-900">
            <div class="space-y-3">
                @forelse($early_warnings as $warning)
                    <div class="flex justify-between items-center bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border-l-4 border-red-500">
                        <div>
                        ...
                        <span class="text-xs text-red-400 font-bold">{{ $warning->last_mem_date ? $warning->last_mem_date->diffForHumans() : '-' }}</span>
                    </div>
                @empty
                    <p class="text-sm text-gray-500 italic text-center py-4 bg-gray-50 dark:bg-gray-800 rounded-lg">Semua santri dalam kondisi baik, rutin setor.</p>
                @endforelse
            </div>
        </x-tahfidz.card>
    </div>
                ...
                @forelse($top_targets as $top)
                    <div>
                        ...
                        <div class="text-[10px] text-gray-400 mt-1">
                            {{ number_format($top->ayat_lancar) }} / {{ number_format($top->total_ayat_target) }} ayat lancar
                        </div>
                    </div>
                @empty
                ...
                <tbody>
                    @forelse($recent_activities as $activity)
                        <tr class="border-b border-gray-50 dark:border-gray-800">
                            <td class="py-2"><span class="font-bold block w-24 truncate">{{ $activity->student->name }}</span><span class="text-[10px] text-gray-400">{{ \Carbon\Carbon::parse($activity->tanggal)->format('d/m/Y') }}</span></td>
                            <td class="py-2 text-gray-600 dark:text-gray-300">Jz.{{ $activity->juz }} {{ $activity->surah }}</td>
                            <td class="py-2 text-right"><span class="text-[10px] uppercase font-bold text-white px-2 py-0.5 rounded {{ $activity->status === 'Lancar' ? 'bg-emerald-500' : 'bg-orange-500' }}">{{ $activity->status }}</span></td>
                        </tr>
                    @empty
                        <tr><td colspan="3" class="text-center italic text-gray-400 py-4">Belum ada data setoran.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </x-tahfidz.card>
                ...
                @forelse($parent_messages as $msg)
                    <div class="p-4 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl">
                        <div class="flex justify-between mb-2">
                            <span class="text-xs font-bold text-emerald-700 dark:text-emerald-500">{{ $msg->sender->name ?? 'Orang Tua' }} — {{ $msg->student->name ?? '' }}</span>
                        ...
                        <form action="{{ route('guru.messages.reply', $msg) }}" method="POST" class="mt-3 flex gap-2">
                            @csrf
                            <input type="text" name="message" required placeholder="Balas pesan..." class="flex-1 rounded-lg border-gray-200 dark:border-gray-700 dark:bg-gray-900 text-sm focus:ring-emerald-500 focus:border-emerald-500">
                            <button type="submit" class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg transition-colors">Balas</button>
                        </form>
                    </div>
                @empty
